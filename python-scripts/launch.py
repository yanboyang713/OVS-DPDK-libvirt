...
'''
def list_and_run():
    for key, value in BASH_SCRIPT_FNS.items() :
        print (key, value)
...
def main():
    ...
    try:
        systemInfo = system()
        if systemInfo.getSystemDistribution() == "arch" and systemInfo.getSystemVersion() == "Manjaro" :
            aboutConfig = config(".config")
            #list_and_run()


        else:
            print_color_string("System does NOT support :(", color = 'red')
